...
import { SupportedGrains } from 'navi-data/models/metadata/time-dimension';
...
export default class ElideFactsAdapter extends EmberObject implements NaviFactAdapter {
  ...
  private readonly grainFormats: Record<SupportedGrains, string> = {
  ...
  private formatTimeValue(value: Moment | string, grain: SupportedGrains) {
  ...
  private buildFilterStr(filters: Filter[]): string {
    const filterStrings = filters.map(filter => {
      ...
      if (type === 'timeDimension') {
        const grain = filter.parameters.grain as SupportedGrains;
